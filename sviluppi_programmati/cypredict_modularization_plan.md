...
- Fase 6 completata in questo ciclo di lavoro: `core/analysis.py` contiene `AnalysisMixin` e `analyze_and_plot`.
...
- Fase 9.2 completata in questo ciclo di lavoro: `core/plotting.py` contiene `PlottingMixin` con i grafici Plotly notebook-oriented usati da `analyze_and_plot` e `multiperiod_analysis`. Il comportamento notebook resta compatibile: i grafici continuano a usare `fig.show()` e il multirange continua a salvare l'HTML diagnostico.
...
- Nota tecnica risolta: `cyGAopt` e `cyGAoptMultiCore` sono ora compilabili da `scripts/build_native.ps1`, espongono `ABI_VERSION = 2`, vengono importati preferendo `native/*/build/lib.*`, e l'import Python rifiuta estensioni native stale. La fitness C++ multicore e' stata allineata a `cyfitness` con test di parita'.
...
- Prossima fase consigliata: completare la pulizia import/commenti e le docstring residue sui moduli gia' estratti, consolidare oggetti risultato/config e aggiornare esempi notebook. Il service e il plotting web per GammaSignalForge restano fuori da questa tranche e andranno progettati separatamente.
