...
- La classe pubblica resta `cyPredict.cyPredict` e ora eredita da `StateMixin`, `DataMixin`, `DatesMixin`, `DetrendingMixin`, `SpectralMixin`, `DiagnosticsMixin`, `IndicatorsMixin`, `ReconstructionMixin` e `ScoringMixin`.
...
- Prossima fase consigliata: estrarre extrema/minmax o spostare `analyze_and_plot` intero in un mixin dedicato, sempre con commit separati e golden QQQ dopo ogni spostamento.
